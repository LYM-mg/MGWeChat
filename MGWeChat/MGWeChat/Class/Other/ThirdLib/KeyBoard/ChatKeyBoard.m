...
{
    return MGScreenH - (frame.origin.y + kChatToolBarHeight);
}
...
+ (instancetype)keyBoardWithNavgationBarTranslucent:(BOOL)translucent
{
    ...
    if (translucent) {
        frame = CGRectMake(0, MGScreenH - kChatToolBarHeight, MGScreenW, kChatKeyBoardHeight);
    }else {
        frame = CGRectMake(0, MGScreenH - kChatToolBarHeight - 64, MGScreenW, kChatKeyBoardHeight);
    }
...
{
    CGRect frame = CGRectMake(0, bounds.size.height - kChatToolBarHeight, MGScreenW, kChatKeyBoardHeight);
    return [[self alloc] initWithFrame:frame];
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _chatToolBar = [[ChatToolBar alloc] initWithFrame:CGRectMake(0, 0, MGScreenW, kChatToolBarHeight)];
        _chatToolBar.delegate = self;
        [self addSubview:self.chatToolBar];
        
        _facePanel = [[FacePanel alloc] initWithFrame:CGRectMake(0, kChatKeyBoardHeight-kFacePanelHeight, MGScreenW, kFacePanelHeight)];
        ...
        _OAtoolbar = [[OfficialAccountToolbar alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.frame), MGScreenW, kChatToolBarHeight)];
...
- (void)keyBoardWillChangeFrame:(NSNotification *)notification
{
    ...
    if (self.chatToolBar.faceSelected && (getSupviewH(self.keyboardInitialFrame) - CGRectGetMidY(self.frame)) < CGRectGetHeight(self.frame))
    {
        [UIView animateWithDuration:0.25 delay:0.03 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            self.frame = CGRectMake(0, getSupviewH(self.keyboardInitialFrame)-CGRectGetHeight(self.frame), MGScreenW, CGRectGetHeight(self.frame));
    ...
    else
    {
        [UIView animateWithDuration:0.25 animations:^{
            ...
            else if (end.origin.y == MGScreenH && begin.origin.y!=end.origin.y && duration > 0)
            {
                //键盘收起
